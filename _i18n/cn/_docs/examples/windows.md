## 用于直接监视 jvm.dll 的示例工具

展示如何使用 Frida 监视由名为 *fledge.exe* (BB Simulator) 的进程执行的 jvm.dll。

将此代码保存为 *bb.py*，运行 BB Simulator (fledge.exe)，然后运行 `python.exe bb.py fledge.exe` 以监视 *jvm.dll* 的 [AES](https://en.wikipedia.org/wiki/Advanced_Encryption_Standard) 使用情况。
